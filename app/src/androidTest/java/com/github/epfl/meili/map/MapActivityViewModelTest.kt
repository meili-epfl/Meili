...
class MapActivityViewModelTest {
    ...
    private fun setupSensorMocks() {
        ...
        Mockito.`when`(
            mockSensorManager.registerListener(
                listenerCaptor.capture(), ArgumentMatchers.any(), ArgumentMatchers.anyInt()
            )
        ).thenReturn(true)

        MapActivityViewModel.getEventValues = { FloatArray(3) { IntMath.pow(-1, it) * (20 * it).toFloat() } }
    ...
    fun sensorsTest() {
        ...
        UiThreadStatement.runOnUiThread {
            ...
            listenerCaptor.value.onSensorChanged(Mockito.mock(SensorEvent::class.java))
        }
    }
}